-- =============================================
-- Bralyx — 003: match_documents (RAG global, sem ACL)
--
-- bx_match_documents(query_embedding, match_count, filter):
...
--     global; com filtro {session_id} retorna só os daquela conversa.
--
...
DROP FUNCTION IF EXISTS bx_match_documents(vector, int, jsonb);
CREATE OR REPLACE FUNCTION bx_match_documents(
...
AS $$
DECLARE
  is_member_caller BOOLEAN := bx_is_member();
...
BEGIN
  ...
  RETURN QUERY
    ...
    FROM bx_documents d
    LEFT JOIN bx_document_metadata m
...
GRANT EXECUTE ON FUNCTION bx_match_documents(vector, int, jsonb)
