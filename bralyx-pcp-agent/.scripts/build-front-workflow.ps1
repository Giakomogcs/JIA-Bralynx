# Builds Bralyx-Front.json (n8n workflow) by embedding front-bralyx.html into an
# HTML template node served via a GET webhook (bralyx-app).
$ErrorActionPreference = 'Stop'

$root      = Split-Path -Parent $PSScriptRoot
$htmlPath  = Join-Path $root 'front-bralyx.html'
$outPath   = Join-Path $root 'workspaces\Bralyx-Front.json'

if (-not (Test-Path $htmlPath)) { throw "front-bralyx.html not found at $htmlPath" }
